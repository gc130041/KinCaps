...
@WebServlet(name = "DetalleCarritoCRUDServlet", urlPatterns = {"/mantenimiento/detallecarrito", "/mantenimiento/detallecarrito/*"})
public class DetalleCarritoCRUDServlet extends HttpServlet {

    private void mostrarFormulario(HttpServletRequest request, HttpServletResponse response) throws IOException {
        response.setContentType("text/html;charset=UTF-8");

        List<Carrito> listaCarritos = new CarritoDAO().listarTodos();
        List<Gorras> listaGorras = new GorrasDAO().listarTodas();
        DetalleCarrito detalleAEditar = (DetalleCarrito) request.getAttribute("detalleEditar");
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("    <title>" + (esEdicion ? "Editar" : "Agregar") + " Detalle de Carrito</title>");
            ...
            out.println("            <h3 class='text-center mb-4'>Formulario para " + (esEdicion ? "editar un" : "agregar un") + " Detalle de Carrito</h3>");
            out.println("            </div>");
            out.println("            <div class='card-body'>");

            String actionUrl = esEdicion ? request.getContextPath() + "/mantenimiento/detallecarrito/actualizar" : request.getContextPath() + "/mantenimiento/detallecarrito/agregar";
            out.println("                <form method='post' action='" + actionUrl + "'>");
            
            if (esEdicion) {
                 out.println("<input type='hidden' name='id' value='" + detalleAEditar.getIdDetalleCarrito()+ "'>");
            }
            
            out.println("    <div class='mb-3'>");
            out.println("        <label class='form-label'>Carrito</label>");
            out.println("        <select class='form-select' name='idCarrito' required>");
            out.println("            <option value=''>Seleccione un carrito</option>");
            for (Carrito carrito : listaCarritos) {
                int id = carrito.getIdCarrito();
                String clienteNombre = (carrito.getCliente() != null) ? carrito.getCliente().getNombre() + " " + carrito.getCliente().getApellido() : "Sin cliente";
                boolean seleccionado = esEdicion && detalleAEditar.getCarrito() != null && detalleAEditar.getCarrito().getIdCarrito() == id;
                out.println("            <option value='" + id + "'" + (seleccionado ? " selected" : "") + ">ID: " + id + " (Cliente: " + clienteNombre + ")</option>");
            }
            out.println("        </select>");
            out.println("    </div>");
            ...
            out.println("                    <div class='mb-3'>");
            out.println("                        <label class='form-label'>Precio Unitario</label>");
            out.println("                        <input type='number' step='0.01' class='form-control' name='precioUnitario' required value='" + (esEdicion && detalleAEditar.getPrecioUnitario() != null ? detalleAEditar.getPrecioUnitario() : "") + "'>");
            out.println("                    </div>");
            ...
            out.println("                        <a href='" + request.getContextPath() + "/mantenimiento/detallecarrito/listar' class='btn btn-secondary'>Cancelar</a>");
            out.println("                    </div>");
            out.println("                </form>");
            out.println("            </div>");
            out.println("        </div>");
            ...
            switch (accion) {
                case "/agregar":
                    if (request.getMethod().equalsIgnoreCase("GET")) {
                        ...
                        Carrito carrito = new CarritoDAO().buscarPorId(idCarrito);
                        Gorras gorra = new GorrasDAO().buscarPorId(idGorra);

                        detalle = new DetalleCarrito(carrito, gorra, cantidad, precioUnitario);
                        dao.guardar(detalle);
                        response.sendRedirect(request.getContextPath() + "/mantenimiento/detallecarrito/listar");
                    }
                    break;
                case "/editar":
                ...
                case "/listar":
                default:
                    List<DetalleCarrito> listaDetalles = dao.listarTodos();
                    request.setAttribute("listaDetalleCarrito", listaDetalles);
                    RequestDispatcher dispatcher = request.getRequestDispatcher("/pages/detalleCarrito.jsp");
                    dispatcher.forward(request, response);
                    break;
            }
        } catch (Exception e) {
            Logger.getLogger(DetalleCarritoCRUDServlet.class.getName()).log(Level.SEVERE, "Error en el servlet", e);
    ...
    public String getServletInfo() {
        return "Servlet para CRUD de DetalleCarrito";
    }
}